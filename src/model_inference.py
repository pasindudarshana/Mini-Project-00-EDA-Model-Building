...
from feature_encoding import OrdinalEncodingStratergy, ServiceFeatureEncodingStrategy
...
class ModelInference:
    ...
    def preprocess_input(self, data: Dict[str, Any]) -> pd.DataFrame:
        ...
        try:
            ...
            if 'tenure' in df.columns:
                logger.info("Applying feature binning for tenure...")
                original_score = df['tenure'].iloc[0]
                ...
                df['tenureBins'] = df['tenure'].apply(assign_bin)
                del df['tenure']  # Drop original tenure column
                binned_score = df['tenureBins'].iloc[0]
                logger.info(f"  ✓ tenure binned: {original_score} → {binned_score} (kept original column)")
            else:
                logger.warning("  ⚠ tenure not found - skipping binning")
            ...
            for col in yes_no_cols:
                if col in df.columns:
                    df[col] = (
                        df[col]
                        #.fillna('No')      # Replace missing values with default 'No'
                        .astype(str)       # Ensure string type
                        .str.strip()       # Remove leading/trailing spaces
                        .str.title()       # Normalize capitalization (e.g., 'yes' -> 'Yes')
                    )
            print("Yes/No columns after cleaning:\n", df[yes_no_cols].head())


            # Apply ordinal encoding
            logger.info("Applying ordinal encoding...")
            ordinal_strategy = OrdinalEncodingStratergy(self.encoding_config['inference_ordinal_mappings'])
            df = ordinal_strategy.encode(df)
            logger.info("  ✓ Ordinal encoding applied")

            # Apply serrvice feature binning
            logger.info("Applying feature binning...")
            service_strategy = ServiceFeatureEncodingStrategy()
            df = service_strategy.encode(df)
            logger.info("  ✓ Service feature binning applied")
            ...
            # Display NaN sum for each column
            print("\nNaN values in each column after processing:")
            print(df.isna().sum())
                
            # Reorder columns to match training data order for model compatibility
            expected_order = [
                'SeniorCitizen','Partner','Dependents','Contract','PaperlessBilling',
                'MonthlyCharges','TotalCharges','tenureBins','gender_Female','gender_Male',
                'PaymentMethod_Bank transfer (automatic)','PaymentMethod_Credit card (automatic)',
                'PaymentMethod_Electronic check','PaymentMethod_Mailed check','PhoneServiceBins',
                'InternetServiceBins'
            ]
    ...
    def predict(self, data: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            churn_confidence = round(probability * 100, 2)
            retain_confidence = round((1 - probability) * 100, 2)
            
            result = {
                "Status": status,
                "Confidence": f"{churn_confidence}%",
                "Retain Confidence": f"{retain_confidence}%"
            }
            ...
            logger.info(f"  • Confidence: {churn_confidence}%")
            logger.info(f"  • Retain Confidence: {retain_confidence}%")
            logger.info(f"{'='*60}\n")
