...
def data_pipeline(
    ...
    try:
        ...
        if 'tenureBins' in df.columns:
            bin_dist = df['tenureBins'].value_counts().to_dict()
            mlflow.log_metrics({f'tenure_bin_{k}': v for k, v in bin_dist.items()})
        
        logger.info("✓ Feature binning completed")
        
        # Feature encoding
        yes_no_cols = ['PaperlessBilling', 'Partner', 'Dependents', 'Churn']
